"""Train GMD baseline for missing modality robustness."""
import os, sys, time, argparse, random, itertools
import numpy as np
...
from models.gmd_baseline import GMD_MSA, gmd_decouple
...
def parse_args():
    ...
    # GMD-specific
    p.add_argument("--gmd_combos", type=int, default=4,
                   help="Number of modality combinations to sample per step")
    p.add_argument("--proj_dims", type=str, default="40,40,40")
    # Output
    p.add_argument("--name", default="gmd_model.pt")
...
def train_epoch(model, optimizer, train_loader, args, epoch):
    ...
    for i_batch, batch in enumerate(train_loader):
        ...
        # GMD Dynamic Sharing: sample k modality combinations per step
        combos = random.sample(ALL_COMBOS, min(args.gmd_combos, len(ALL_COMBOS)))
        if [0, 1, 2] not in combos:
            combos[0] = [0, 1, 2]

        optimizer.zero_grad(set_to_none=True)
        total_loss = 0.0
        for combo in combos:
            output, _ = model.forward_combination(x, combo)
            loss = l1(output, labels)
            total_loss = total_loss + loss

        total_loss = total_loss / len(combos)
        total_loss.backward()
        ...
        if i_batch % args.log_interval == 0 and i_batch > 0:
            ...
            msg = (f"E {epoch:2d} | B {i_batch:4d}/{n_batches:4d} | "
                   ...
                   f"Loss {epoch_loss/max(i_batch,1):.4f} | combos={len(combos)}")
            print(msg)
            start = time.time()
...
def main():
    ...
    print(f"GPU: {use_cuda} | GMD | combos={args.gmd_combos}")
    ...
    model = GMD_MSA(orig_dim=orig_dim, proj_dims=proj_dims, out_dropout=args.dropout_prob)
