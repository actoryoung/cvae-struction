"""Train TMDC baseline (AAAI 2026) for missing modality robustness."""
...
from models.tmdc_baseline import TMDC_MSA
...
def parse_args():
    ...
    p.add_argument("--name", default="tmdc_model.pt")
...
def train_epoch(model, optimizer, train_loader, args, epoch):
    ...
    for i_batch, batch in enumerate(train_loader):
        ...
        # Drop modality forward (TMDC denoising + complementation)
        output_drop, h_complement, h_true, h_common = model.forward_with_dropout(x, drop_idx)
        loss_drop = l1(output_drop, labels)
        loss_recon = F.mse_loss(h_complement, h_true)
...
def main():
    ...
    print(f"GPU: {use_cuda} | TMDC (AAAI 2026) | recon={args.recon_weight}")
    ...
    model = TMDC_MSA(orig_dim=orig_dim, proj_dims=proj_dims, out_dropout=args.dropout_prob)
