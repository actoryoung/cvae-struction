"""Train MissModal baseline for missing modality robustness."""
...
from models.missmodal_baseline import MissModal_MSA, missmodal_train_step
...
def parse_args():
    ...
    p.add_argument("--text_dropout", type=float, default=0.0,
                   help="T-specific dropout prob (0=uniform 1/3 per modality)")
    # MissModal-specific hyperparams
    p.add_argument("--miss_alpha", type=float, default=0.5, help="Contrastive loss weight")
    p.add_argument("--miss_beta", type=float, default=0.3, help="Distribution loss weight")
    p.add_argument("--miss_gamma", type=float, default=0.5, help="Sentiment alignment weight")
    p.add_argument("--miss_temp", type=float, default=0.07, help="Contrastive temperature")
    p.add_argument("--proj_dims", type=str, default="40,40,40")
    # Output
    p.add_argument("--name", default="missmodal_model.pt")
...
def train_epoch(model, optimizer, train_loader, args, epoch):
    ...
    start = time.time()
    ep_total, ep_reg_full, ep_reg_drop = 0.0, 0.0, 0.0
    ep_contrast, ep_dist, ep_sent = 0.0, 0.0, 0.0
    ...
    for i_batch, batch in enumerate(train_loader):
        ...
        total, reg_full, reg_drop, lc, ld, ls = missmodal_train_step(
            model, x, labels, drop_idx, l1)
        ...
        ep_total += total.item()
        ep_reg_full += reg_full.item()
        ep_reg_drop += reg_drop.item()
        ep_contrast += lc.item()
        ep_dist += ld.item()
        ep_sent += ls.item()
        ...
        if i_batch % args.log_interval == 0 and i_batch > 0:
            ...
            msg = (f"E {epoch:2d} | B {i_batch:4d}/{n_batches:4d} | "
                   ...
                   f"Contr {ep_contrast/max(i_batch,1):.4f} | "
                   f"Dist {ep_dist/max(i_batch,1):.6f} | "
                   f"Sent {ep_sent/max(i_batch,1):.4f}")
            print(msg)
            start = time.time()
...
def main():
    ...
    print(f"GPU: {use_cuda} | MissModal | α={args.miss_alpha} β={args.miss_beta} γ={args.miss_gamma}")
    ...
    model = MissModal_MSA(
        orig_dim=orig_dim, proj_dims=proj_dims, out_dropout=args.dropout_prob,
        alpha=args.miss_alpha, beta=args.miss_beta, gamma=args.miss_gamma,
        temperature=args.miss_temp,
    )
